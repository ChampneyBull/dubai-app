...
const Scoreboard = ({ golfers, onLogWinnings, onAdminOpen, user, onLogout }) => {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#0c140c] text-white p-6 max-w-sm mx-auto">
                    ...
                    <div className="bg-gradient-to-br from-[#1b5e20] to-[#2e7d32] rounded-[2rem] p-6 mb-8 relative overflow-hidden shadow-2xl shadow-green-900/20">
                        <div className="relative z-10">
                            ...
                            <div className="flex items-baseline space-x-2">
                                <span className="text-5xl font-black text-white">£{leader.earnings.toLocaleString()}</span>
                            </div>
                        </div>
                            ...
                            <div key={golfer.id} className="bg-white/5 rounded-3xl p-4 flex items-center space-x-4 border border-white/5">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-gray-500 text-[10px] uppercase font-bold tracking-wider italic">
                                        {index === 0 ? 'Leading' : `£${(leader.earnings - golfer.earnings).toLocaleString()} Behind`}
                                    </p>
                                </div>
                                <div className="text-right">
                                    <span className="font-black text-lg">£{golfer.earnings.toLocaleString()}</span>
                                </div>
                            </div>
                        ))}
                    </div>
                                            ...
                                            <div className="flex flex-col items-center">
                                                ...
                                                <div className="w-14 h-14 translate-y-[-2px]">
                                                    <img
                                                        src={golfer.image}
                                                        alt={golfer.name}
                                                        className="w-full h-full object-contain filter drop-shadow-[0_8px_15px_rgba(0,0,0,0.5)]"
                                                    />
                                                </div>
                                            </div>
                                        </div>
